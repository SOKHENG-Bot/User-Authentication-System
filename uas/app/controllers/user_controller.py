...
async def logout_account(
    ...
    return await TokenService(session).revoke_token(
        authorize, response, all_sessions=False
    )
...
@user_router.get("/profile", response_model=UserProfile, status_code=status.HTTP_200_OK)
...
@user_router.delete("/account", status_code=status.HTTP_200_OK)
...
@user_router.get(
    "/sessions", response_model=List[LoginProfile], status_code=status.HTTP_200_OK
)
...
@user_router.delete("/sessions/{id}", status_code=status.HTTP_200_OK)
